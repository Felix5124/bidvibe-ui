...
export default function NotificationsPage() {
  ...
  const loadData = useCallback(async (targetPage) => {
    ...
    try {
      ...
      setUnreadCount(readUnreadCount(readApiData(countRes)));
    } catch (err) {
  ...
  const handleMarkOne = async (id) => {
    try {
      ...
      console.error(
        "[NotificationsPage] Failed to mark notification as read",
        err,
      );
  ...
  const handleMarkAll = async () => {
    try {
      ...
      console.error(
        "[NotificationsPage] Failed to mark all notifications as read",
        err,
      );
      setError(
        err?.response?.data?.message || "Đánh dấu tat ca đã đọc thất bại.",
      );
    }
  };
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="mb-4 bg-white border border-gray-200 rounded-xl p-4 flex items-center justify-between">
          <p className="text-gray-700">
            Chưa đọc: <span className="font-semibold">{unreadCount}</span>
          </p>
          <button
            type="button"
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {notifications.map((noti) => (
              <div
                key={noti.id}
                className={`bg-white border rounded-xl p-4 ${noti.read ? "border-gray-200" : "border-blue-300"}`}
              >
