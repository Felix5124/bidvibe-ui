...
export default function MarketDetailPage() {
  ...
  const isSeller = useMemo(
    () => user?.id && listing?.seller?.id && user.id === listing.seller.id,
    [listing, user],
  );
  const isBuyer = useMemo(
    () => user?.id && listing?.buyer?.id && user.id === listing.buyer.id,
    [listing, user],
  );
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        setError(
          err?.response?.data?.message || "Không tải được chi tiết listing.",
        );
      } finally {
        setLoading(false);
      }
                  ...
                  <div className="mt-6 bg-emerald-50 border border-emerald-200 rounded-xl p-4">
                    ...
                    <p className="text-sm text-emerald-700 mb-3">
                      ...
                      <span className="font-medium">
                        {listing.seller.nickname || "người dùng"}
                      </span>{" "}
                      về giao dịch này.
                    </p>
                    <Link
                ...
                {listing.status === "SOLD" && isSeller && listing.buyer && (
                  <div className="mt-6 bg-blue-50 border border-blue-200 rounded-xl p-4">
                    ...
                    <p className="text-sm text-blue-700 mb-3">
                      Người mua:{" "}
                      <span className="font-medium">
                        {listing.buyer.nickname || "người dùng"}
                      </span>{" "}
                      với giá {formatVND(listing.askingPrice)}
                    </p>
                    <Link
